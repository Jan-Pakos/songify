...
class AlbumInfoTestImpl implements AlbumInfo {
    ...
    public Set<SongInfo> getSongs() {
        return album.getSongs().stream().map(SongInfoTestImpl::new).collect(Collectors.toSet());
    }

    @Override
    public Set<ArtistInfo> getArtists() {
        return album.getArtists().stream().map(ArtistInfoTestImpl::new).collect(Collectors.toSet());
    }
}
